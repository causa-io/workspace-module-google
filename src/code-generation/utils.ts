...
export function schemaMatchesGlobPatterns(
  ...
  const workspaceContext = renderer.targetLanguage.workspaceContext;
  if (!workspaceContext || !workspaceContext.getProjectPathOrThrow) {
    return true;
  }

  const projectPath = workspaceContext.getProjectPathOrThrow();
